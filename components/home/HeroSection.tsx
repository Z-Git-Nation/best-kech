...
// Placeholder gradient backgrounds — remplacer par vraies photos villa
const BG_GRADIENTS = [
...
export default function HeroSection() {
  ...
  const handleToggle = (newIdx: number) => {
  ...
  // Direction: going forward (0→1) exits LEFT enters RIGHT; backward (1→0) exits RIGHT enters LEFT
  const goingForward = activeIdx > prevIdxRef.current;
  const exitX   = goingForward ? -120 : 120;
  const enterX  = goingForward ?  120 : -120;
  ...
  return (
    ...
    >
      {/* Background photo / gradient — swap animé */}
      ...
      {/* Bokeh overlay (simule le flou de fond) */}
      <div style={{
        position: 'absolute', inset: 0, zIndex: 1, pointerEvents: 'none',
        background: 'radial-gradient(ellipse 80% 60% at 30% 60%, rgba(44,24,16,.3) 0%, rgba(44,24,16,.6) 60%, rgba(44,24,16,.85) 100%)',
      }} />
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          {/* 3D Viewer — plus grand pour éviter le clipping */}
          <div style={{ position: 'relative', height: 'clamp(320px, 45vw, 540px)' }}>
            <AnimatePresence mode="wait" custom={goingForward}>
              <motion.div
                key={activeIdx}
                custom={goingForward}
        ...
        <div style={{
          ...
          boxShadow:    '0 24px 80px rgba(44,24,16,.45)',
        }}>
          <h3 style={{ fontFamily: 'var(--font-cormorant)', fontSize: '1.6rem', color: 'var(--brown-deep)', marginBottom: '1.5rem' }}>
          ...
          >
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
              {(['Arrivée', 'Départ'] as const).map(label => (
                <label key={label} style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                  <span style={{ fontFamily: 'var(--font-inter)', fontSize: '0.68rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-mid)' }}>{label}</span>
                  <input type="date" style={{ fontFamily: 'var(--font-inter)', fontSize: '0.88rem', padding: '0.6rem 0.8rem', borderRadius: 10, border: '1.5px solid rgba(196,113,74,.25)', color: 'var(--brown-deep)', background: 'var(--cream-warm)', outline: 'none' }} />
                </label>
              ))}
            </div>

            <label style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
              <span style={{ fontFamily: 'var(--font-inter)', fontSize: '0.68rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-mid)' }}>Voyageurs</span>
              <select style={{ fontFamily: 'var(--font-inter)', fontSize: '0.88rem', padding: '0.6rem 0.8rem', borderRadius: 10, border: '1.5px solid rgba(196,113,74,.25)', color: 'var(--brown-deep)', background: 'var(--cream-warm)', outline: 'none', cursor: 'pointer' }}>
                {[1,2,4,6,8,10,12].map(n => <option key={n} value={n}>{n} {n === 1 ? 'voyageur' : 'voyageurs'}</option>)}
              </select>
            </label>

            <label style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
              <span style={{ fontFamily: 'var(--font-inter)', fontSize: '0.68rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-mid)' }}>Budget / nuit</span>
              <select style={{ fontFamily: 'var(--font-inter)', fontSize: '0.88rem', padding: '0.6rem 0.8rem', borderRadius: 10, border: '1.5px solid rgba(196,113,74,.25)', color: 'var(--brown-deep)', background: 'var(--cream-warm)', outline: 'none', cursor: 'pointer' }}>
            ...
            <button
              type="submit"
              style={{ marginTop: '0.25rem', background: 'var(--terracotta)', color: 'var(--cream)', border: 'none', borderRadius: 12, padding: '0.9rem', fontFamily: 'var(--font-inter)', fontSize: '0.85rem', letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 500, cursor: 'pointer', transition: 'background .25s' }}
      ...
      <div style={{ position: 'absolute', bottom: '2rem', left: '50%', transform: 'translateX(-50%)', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.4rem' }}>
        <span style={{ fontFamily: 'var(--font-inter)', fontSize: '0.62rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(250,247,242,.4)' }}>Découvrir</span>
        <motion.div
          animate={{ y: [0, 6, 0] }}
